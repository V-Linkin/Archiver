...
```text
windows/src/Gatherly.Windows/Views/ImageViewerWindow.axaml — 新增图片查看器窗口
windows/src/Gatherly.Windows/Views/ImageViewerWindow.axaml.cs — 图片查看器逻辑（多图导航、键盘操作）
windows/src/Gatherly.Windows/ViewModels/MainWindowViewModel.cs — 新增 OpenImageViewer/OpenVideoFile/OpenVideoFolder 命令
windows/src/Gatherly.Windows/Views/ItemDetailView.axaml — 图片可点击、视频增加打开按钮
```

结论：

* 详情页图片缩略图支持点击查看大图
* 多图支持上一张/下一张（◀ ▶ 按钮 + 左右箭头键）
* ESC 关闭查看器
* 视频支持用系统默认播放器打开
* 视频支持打开所在文件夹并定位文件
* 文件不存在时不崩溃
...
* Windows MVP 功能完整，可作为 Release Candidate
* 148 个测试全部通过
* macOS 项目不受影响
...
### Phase 7C：ImportService 真实导入基础链路 ✅
...
```text
windows/src/Gatherly.Windows/Services/Import/ImportStatus.cs — 导入状态枚举
windows/src/Gatherly.Windows/Services/Import/ImportResult.cs — 导入结果模型
windows/src/Gatherly.Windows/Services/Import/ImportRequest.cs — 导入请求模型
windows/src/Gatherly.Windows/Services/Parsers/IContentParser.cs — 解析器接口
windows/src/Gatherly.Windows/Services/Parsers/ParseResult.cs — 解析结果 + ParsedContent + ParseRequest
windows/src/Gatherly.Windows/Services/Parsers/PlatformRouter.cs — 平台路由器
windows/src/Gatherly.Windows/Services/Parsers/NotImplementedParser.cs — 未实现解析器占位
windows/src/Gatherly.Windows/Database/ImportTaskRepository.cs — import_tasks CRUD
windows/tests/Gatherly.Windows.Tests/Services/ImportServiceTests.cs — 12 个测试
```

改动：

```text
windows/src/Gatherly.Windows/Services/ImportService.cs — 完整导入管道（去重 + 创建任务 + Router）
windows/src/Gatherly.Windows/ViewModels/HomeViewModel.cs — ImportLink 改为 async
windows/src/Gatherly.Windows/ViewModels/MainWindowViewModel.cs — 传递 ItemRepository + ImportTaskRepository
windows/src/Gatherly.Windows/Database/ItemRepository.cs — 新增 GetByNormalizedUrlAsync
```

结论：

* ImportService 完整管道：URL 提取 → 平台识别 → Normalize → Duplicate 检查 → 创建 import_task → Router → Parser
* 支持 import_task 创建和状态更新
* 支持 duplicate 检查（items + import_tasks）
* 新增 PlatformRouter / IContentParser / NotImplementedParser 骨架
* 当前所有平台统一返回 ParserNotImplemented，但任务已创建
* 后续 Phase 7D 开始逐个接入真实 Parser
* 234 个测试全部通过
* macOS 项目不受影响
