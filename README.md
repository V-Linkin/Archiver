...
- **平台列表分页**：内容超过 60 条时自动分页，支持页码输入跳转
- **主动切页滚动修复**：翻页、跳转页码后自动回到新页面顶部
- **详情返回位置保持**：从内容详情返回列表时，恢复到刚才浏览的位置
- **删除后自动补位**：删除内容后当前页自动补齐，保持在删除位置附近
- **切换平台关闭详情**：切换平台时自动关闭详情页，新平台回到第 1 页
- **平台列表稳定性**：修复切换平台后内容不显示、返回详情闪白等问题
